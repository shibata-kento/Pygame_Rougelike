...
class MapManager:
    ...
    def draw_floor_info(self):
        ...
        floor_text = f'{self.player.map_coordinate[0]}, {self.player.map_coordinate[1]}'
        game_system.add_draw_object(
            Text(floor_text, (8, 8), Color.WHITE, Text.FontSize.SMALL)
        )
        speed_text = '[S]peed ' + str(game_system.speed)
        game_system.add_draw_object(
            Text(speed_text, (100, 8), Color.WHITE, Text.FontSize.SMALL)
        )
        if game_info.floor_info_view_time <= 0:
            return 

        game_info.decrement_floor_info_view_time()
        floor_info_text = f'B {game_info.floor} F'
        game_system.add_draw_object(
            Text(floor_info_text, (56, 40), Color.CYAN)
        )
    ...
    def draw_map(self):
        ...
        self.event_manager.draw(game_system, center, dungeon.floor_map)
        self.player.draw()
    ...
    def __draw_floor_info(self, base_position: (int, int)):
        ...
        texts = [
            f'B{floor}F',
            f'LV{player.stats.level}',
            f'STR{player.stats.strength}'
        ]
        text = ' '.join(texts)

        self.game_system.add_draw_object(
            Text(text, (x, y), Color.CYAN, Text.FontSize.SMALL)
        )
    ...
    def __draw_player_info(self, base_position: (int, int)):
        ...
        self.game_system.add_draw_object(
            Text(text, (x, y+12), color, Text.FontSize.SMALL)
        )
    ...
    def __draw_item_info(self, base_position: (int, int)):
        ...
        self.game_system.add_draw_object(
            Text(f'[ P ] {stats.potion}', (x, y), Color.GREEN, Text.FontSize.SMALL)
        )

        self.game_system.add_draw_object(
            Text(f'[ B ] {stats.bom}', (x, y+12), Color.GREEN, Text.FontSize.SMALL)
        )
    ...
    def draw_parameter(self):
        ...
        game_system.add_draw_object(
            Rect((0, base_y-2), (144, 144-base_y + 2), is_absolute_position=True)
        )

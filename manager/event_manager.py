...
class EventManager:
    ...
    def __init__(self, game_system: GameSystem, game_info: GameInfo, player: Player):
    ...
    def create_events(self, dungeon: Dungeon):
        ...
        for _ in range(60):
            ...
            if choice == 1:
                treasure = TreasureBox((y, x), self.__game_system, self.__game_info)
                self.__treasure_map[treasure.map_coordinate] = treasure
            else:
                enemy = Enemy((y, x), self.__game_system, self.__game_info, dungeon, player, self.__enemy_map)
                self.__enemy_map[enemy.map_coordinate] = enemy
    
    def draw(self, game_system: GameSystem, center: (int, int), floor_map: Matrix):

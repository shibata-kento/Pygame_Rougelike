...
class Enemy(Event):
    ...
    def __init__(self,
        ...
        self.__debug_text = f'{self.__stats.max_hp}'
    ...
    def __get_default_stats(self) -> Stats:
        ...
        max_hp = 60 * (self.type + math.ceil(level / 2)) + (level - 1) * 10
    ...
    def battle(self):
        damage = self.__player.stats.strength + random.randint(
            1,
    ...
    def __do_move(self):
        ...
        position = (self.y + float(y) * value, self.x + float(x) * value)
    ...
    def draw(self):
        self.__anime_frame += 1
        position = self.__game_info.convert_map_to_display((self.x, self.y))

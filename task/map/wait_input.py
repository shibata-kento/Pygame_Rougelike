...
class WaitInput(Task):
    ...
    def update(self):
        ...
        if InputManager.isPush(Key.p) and player.stats.potion > 0:
            from task.map.use_item import UseItem
            self.__next_task = UseItem(self.__map_manager, Item(Item.Type.POTION))
            return

        if InputManager.isPush(Key.b) and player.stats.bom > 0:
            from task.map.use_item import UseItem
